...
angular
  ...
  .factory('Data', function() {
    ...
    return {
      ...
      getWC: function(text) {
        // console.log( text.match(/(\w+)\b/g), text)
      ...
      annotatedText: function(text, feedback, comments) {
          // console.log(feedback);
          var newText = text;
          // console.log(text, feedback, comments)
          ...
          comments.forEach( function(obj,id) {
            var comment = obj.comment
          // find examples and get errors            // find error then display it'
            if (obj.example) {
            var example = new RegExp('('+obj.example+')','gi');
              // console.log(example, obj);
              // console.log(text.match(example))
            var rplString = '$1</span> <strong>[2.'+(id+1)+']</strong>';
              newText = newText.replace(example,'<span class="inline-error">'+rplString);
            }
          });
